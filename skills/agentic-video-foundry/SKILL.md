...
description: Orchestrate Agentic Video Foundry, a complete and repeatable short-form video workflow from creative brief, script and art direction through renderer and scene routing, ElevenLabs voice/music/SFX generation, Text-to-Lottie assets, timestamped captions, deterministic animation, audio mixing, rendering, visual and technical QA, delivery, and learning capture. Use when Codex is asked to make, revise, polish, render, or standardize a vertical video for Xiaohongshu, Douyin, TikTok, Reels, Shorts, product demos, knowledge sharing, or social promotion, especially when the request includes narration, background music, sound effects, programmatic motion graphics, reusable scenes, or repeated future production.
---
...
2. Write a one-sentence communication goal, audience, platform, target duration, CTA, emotional arc, and explicit exclusions. Turn the narration into beats; do not narrate every on-screen list.
...
4. Define the visual system and timing before polishing scenes: aspect ratio, safe zones, type scale, palette, motion tokens, scene map, one hero per frame, intentional rests, and one or two peak moments.
5. Secure credentials before paid generation. Read secrets from Keychain or environment variables, never source, prompts, manifests, logs, or shell history. Generate low-cost voice auditions before the full narration. Save provider, model, voice, prompt, duration, seed where supported, and content hashes in a secret-free manifest.
6. Generate narration by semantic segment. Use provider timestamps to derive the master scene timeline and captions. Treat mixed Latin phrases as atomic subtitle units. Regenerate only failed or changed segments.
7. Generate music to the story arc and sound effects only for meaningful transitions. Mix against the actual voice, not fixed percentages copied from another project. Keep music clearly audible on phone speakers while preserving speech intelligibility.
8. Build deterministic frame-driven visuals. Centralize timeline, theme, captions, and audio cues. Render representative stills before a full encode and inspect the exact frames where overlays, transitions, and platform-safe areas are most likely to fail.
9. Render the complete video, then run the audit script and the manual checks in the QC checklist. Any voice, music, SFX, timing, or gain change invalidates previous audio measurements and requires a fresh full render and master check.
10. Deliver the final video, cover, captions, editable source, secret-free manifest, final script, publish copy, and concise reproduction commands. Record only verified, reusable surprises in `.learnings/`; promote recurring patterns to project rules or a skill.
